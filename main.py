...
pcs = set()
# session_id -> {"pc": peer_connection, "ws": websocket, "latency_tracker": tracker}
webrtc_sessions = {}
    ...
    def measure(self, start_event: str, end_event: str, metric_name: str):
        ...
        if start_event in self.timestamps and end_event in self.timestamps:
            latency = (self.timestamps[end_event] -
                       self.timestamps[start_event]) * 1000  # ms
...
class AudioTransformTrack(MediaStreamTrack):
        ...
        try:
            ...
            if self.groq_api_key:
                try:
                    ...
                    logger.error(
                        f"❌ Failed to initialize Groq: {e}", exc_info=True)
            else:
            ...
            if self.deepgram_api_key:
                try:
                    ...
                    deepgram_client = DeepgramClient(
                        self.deepgram_api_key, config)
                    self.deepgram_connection = deepgram_client.listen.live.v(
                        "1")

                    # Set up event handlers
                    self.deepgram_connection.on(
                        LiveTranscriptionEvents.Open, self.on_deepgram_open)
                    self.deepgram_connection.on(
                        LiveTranscriptionEvents.Transcript, self.on_deepgram_message)
                    self.deepgram_connection.on(
                        LiveTranscriptionEvents.Error, self.on_deepgram_error)
                    ...
                    options = LiveOptions(
                        ...
                        sample_rate=48000,    # Match browser audio (48kHz)
                        # Mono from browser (after conversion)
                        channels=1,
                    ...
                    logger.error(
                        f"❌ Failed to initialize Deepgram: {e}", exc_info=True)
    ...
    def on_deepgram_message(self, *args, **kwargs):
        ...
        try:
            # Extract result from kwargs (Deepgram SDK passes it differently)
            result = kwargs.get('result', args[0] if args else None)
            if not result:
                logger.error("No result in Deepgram message")
                return

            logger.debug(f"📥 Deepgram message received: {result}")

            sentence = result.channel.alternatives[0].transcript
            if len(sentence) == 0:
                logger.debug("📭 Empty transcript (silence or processing)")
                return
    ...
    def on_deepgram_error(self, *args, **kwargs):
        ...
        logger.error(f"Error type: {type(error)}, kwargs: {kwargs}")
        asyncio.run_coroutine_threadsafe(
            ...
            for chunk in stream:
                if chunk.choices[0].delta.content:
                    ...
                    if first_token:
                        ...
                        llm_latency = self.latency_tracker.measure(
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    tts_latency = self.latency_tracker.measure(
    ...
    async def recv(self):
        ...
        if not self.latency_tracker.timestamps.get("audio_received"):
            ...
            logger.info(
                f"🎤 First audio frame received: format={frame.format}, samples={frame.samples}, sample_rate={frame.sample_rate}")
        ...
        if self.deepgram_connection:
            try:
                ...
                if self._frame_count < 3:
                    logger.info(
                        f"📊 Frame #{self._frame_count + 1}: shape={audio_array.shape}, dtype={audio_array.dtype}, samples={frame.samples}, channels={audio_array.shape[0] if len(audio_array.shape) > 1 else 1}")
                ...
                if len(audio_array.shape) > 1 and audio_array.shape[0] == 2:
                    # Stereo: average both channels to create mono
                    logger.info(
                        f"🔊 Converting stereo to mono (shape: {audio_array.shape})")
                    audio_array = np.mean(audio_array, axis=0).astype(np.int16)
                    logger.info(
                        f"🔊 After conversion: shape={audio_array.shape}")
                elif len(audio_array.shape) > 1 and audio_array.shape[0] == 1:
                    # Single channel but needs reshaping
                    # Shape (1, N) means 1 channel with N values
                    # But frame.samples tells us the ACTUAL number of samples
                    # If shape[1] != frame.samples, the data might be interleaved
                    audio_array = audio_array.flatten()

                    # Check if we need to downsample
                    if len(audio_array) != frame.samples:
                        logger.info(
                            f"🔊 Audio array length ({len(audio_array)}) != samples ({frame.samples}), taking every 2nd sample")
                        # Data is likely interleaved stereo stored as mono
                        # Take every 2nd sample to get true mono
                ...
                if self._frame_count <= 3:
                    logger.info(
                        f"🔊 Sending {len(audio_bytes)} bytes to Deepgram")
                ...
                if self._frame_count % 100 == 0:
                    logger.info(
                        f"📊 Sent {self._frame_count} frames to Deepgram ({len(audio_bytes)} bytes each)")

            except Exception as e:
                logger.error(
                    f"❌ Error processing audio frame #{self._frame_count}: {e}", exc_info=True)
        else:
            if not hasattr(self, '_warned_no_deepgram'):
                self._warned_no_deepgram = True
                logger.warning(
                    "⚠️ Deepgram connection not initialized, audio frames not being processed")
...
async def root():
    ...
    deepgram_status = "configured" if os.getenv(
        "DEEPGRAM_API_KEY") else "not configured"
    groq_status = "configured" if os.getenv(
        "GROQ_API_KEY") else "not configured"
    return {
...
async def webrtc_offer(request: Request):
    ...
    try:
        ...
        async def on_track(track):
            ...
            if track.kind == "audio":
                logger.info(
                    "🎤 Audio track received, waiting for WebSocket connection...")
                ...
                audio_track = AudioTransformTrack(
                    track, websocket, latency_tracker)
                    ...
                    try:
                        ...
                        while True:
                            ...
                            if frame_count % 100 == 0:  # Log every 100 frames
                                logger.info(
                                    f"📊 Processed {frame_count} audio frames, ~{total_bytes} bytes")
    ...
    async def text_to_speech(text: str):
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                logger.info(
                    f"🔊 Received {len(audio_data)} bytes of audio from Deepgram TTS")
                ...
                logger.info(
                    f"🔊 TTS complete: Sent {total_chunks} audio chunks ({len(audio_data)} bytes)")
                ...
                if chunk.choices[0].delta.content:
                    ...
                    if any(punct in content for punct in ['. ', '! ', '? ', '.\n', '!\n', '?\n']):
                        ...
                        if sentence_to_speak:
                            sentence_count += 1
                            logger.info(
                                f"🔊 Sentence #{sentence_count} complete: '{sentence_to_speak[:50]}...'")
    ...
    try:
        ...
        while True:
            ...
            if "bytes" in data:
                ...
                logger.info(
                    f"Received audio chunk #{audio_chunks_received}, size: {len(audio_data)} bytes")
                ...
                if deepgram_connection:
                    try:
                        ...
                        if audio_chunks_received % 10 == 0:  # Log every 10 chunks
                            logger.info(
                                f"✅ Sent chunk #{audio_chunks_received} to Deepgram ({len(audio_data)} bytes)")
                    except Exception as e:
                    ...
                    if audio_chunks_received == 1:
                        logger.warning(
                            "⚠️ Deepgram connection not initialized, audio not being transcribed")
                ...
                if message == "start_recording":
                    logger.info(
                        "Recording started by client - initializing Deepgram and Groq")

                    # Initialize Groq client
                    if groq_api_key:
                        try:
                            logger.info(
                                f"🔧 Initializing Groq with API key: {groq_api_key[:10]}...")
                        ...
                        logger.warning(
                            "⚠️ groq_api_key is None or empty - cannot initialize Groq client")
                    ...
                    if deepgram_api_key:
                        try:
                            ...
                            deepgram_connection = deepgram_client.listen.live.v(
                                "1")
                            ...
                            def on_open(self, open_event, **kwargs):
                                ...
                                logger.info(
                                    "   Status: Ready to receive audio and send transcripts")
                                logger.info("=" * 60)
                            ...
                            def on_message(self, result, **kwargs):
                                try:
                                    ...
                                    if len(sentence) == 0:
                                        logger.debug(
                                            "📭 Empty transcript received (silence or processing)")
                                    else:
                                        is_final = result.is_final
                                        ...
                                        if is_final:
                                            # Mark timestamp for latency tracking
                                            latency_tracker.mark(
                                                "transcript_received")

                                            # Measure STT latency
                                            stt_latency = latency_tracker.measure(
                                                "audio_received", "transcript_received", "stt_latency")
                                            if stt_latency:
                                                logger.info(
                                                    f"📊 STT Latency: {stt_latency:.2f}ms")

                                        # Detailed console logging
                                        logger.info("=" * 60)
                                        logger.info(
                                            f"📝 DEEPGRAM TRANSCRIPT RECEIVED:")
                                        logger.info(
                                            f"   Type: {'FINAL' if is_final else 'INTERIM'}")
                                        logger.info(f"   Text: '{sentence}'")
                                        logger.info(
                                            f"   Confidence: {confidence:.2%}" if confidence else "   Confidence: N/A")
                                        logger.info("=" * 60)

                                        if is_final:
                                            # FINAL transcript - send to LLM for response
                                            logger.info(
                                                "🎯 Final transcript detected - sending to Groq LLM")
                                    ...
                                    logger.error(
                                        f"❌ Error parsing transcript result: {e}")
                                    logger.error(
                                        f"📄 Result object type: {type(result)}")
                                    logger.error(f"📄 Result object: {result}")
                                except Exception as e:
                                    logger.error(
                                        f"❌ Error processing transcript: {e}", exc_info=True)

                            def on_metadata(self, metadata, **kwargs):
                                logger.info(f"📊 DEEPGRAM METADATA: {metadata}")
                                logger.debug(
                                    f"   Full metadata object: {metadata}")
                            ...
                            deepgram_connection.on(
                                LiveTranscriptionEvents.Open, on_open)
                            deepgram_connection.on(
                                LiveTranscriptionEvents.Transcript, on_message)
                            deepgram_connection.on(
                                LiveTranscriptionEvents.Metadata, on_metadata)
                            deepgram_connection.on(
                                LiveTranscriptionEvents.Error, on_error)
                            deepgram_connection.on(
                                LiveTranscriptionEvents.Close, on_close)
                            ...
                            logger.info(
                                f"   Interim Results: {options.interim_results}")
                            logger.info("=" * 60)
                            ...
                            if deepgram_connection.start(options) is False:
                                ...
                                logger.info(
                                    "🎤 Waiting for 🟢 OPEN event and audio data...")
        ...
        logger.info(
            f"Client disconnected. Chunks received: {audio_chunks_received}")
